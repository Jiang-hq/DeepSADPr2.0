...
def ZSCALE(sequence):
    zscale = pd.read_csv(ZSCALE_path)
    encodings = []
    for i in sequence:
        seqLine = list(i)
        code = []
...
train_encodings = np.array(ZSCALE(x_train))
test_encodings = np.array(ZSCALE(x_ind))


x_train = train_encodings.reshape(train_encodings.shape[0],41,5)

x_ind = test_encodings.reshape(test_encodings.shape[0],41,5)

shape = x_train.shape[1:]
shape1 = x_ind.shape[1:]
...
for train_index, test_index in kf.split(x_train):  # 将训练集进行10折交叉验证
    # print('train_index:%s ' %(train_index[:10]))
